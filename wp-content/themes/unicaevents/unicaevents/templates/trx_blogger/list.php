...
if ( !function_exists( 'unicaevents_template_list_theme_setup' ) ) {
	add_action( 'unicaevents_action_before_init_theme', 'unicaevents_template_list_theme_setup', 1 );
	function unicaevents_template_list_theme_setup() {
		unicaevents_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'unicaevents')
			));
	}
}

// Template output
if ( !function_exists( 'unicaevents_template_list_output' ) ) {
	function unicaevents_template_list_output($post_options, $post_data) {
