...
Version: 0.6.1 (modified by UnicaEvents - no backward compatibility!)
Text Domain: unicaevents
...
class WP_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		@set_time_limit(max(30, min(1800, (int) unicaevents_get_theme_option('admin_dummy_timeout'))));
		...
		if ($this->debug) dfl(esc_html__('Process authors', 'unicaevents'));
		...
		try {
			...
			$error = true;
			dfl( sprintf(esc_html__('Error while import: %s', 'unicaevents'), $e->getMessage()) );
	...
	function import_start( $file ) {
		...
		if ($this->debug) dfl(esc_html__('Start import posts', 'unicaevents'));

		if ( ! is_file($file) ) {
			dfl( sprintf(esc_html__('Sorry, there has been an error: %s', 'unicaevents'), esc_html__( 'The file does not exist, please try again.', 'unicaevents')) );
			$this->footer();
			die();
		}
		...
		if ( is_wp_error( $import_data ) ) {
			dfl( sprintf(esc_html__('Sorry, there has been an error: %s', 'unicaevents'), $import_data->get_error_message()) );
			$this->footer();
			die();
		}
	...
	function import_end() {
		...
		if ($this->debug) dfl(esc_html__('Finish import posts', 'unicaevents'));
		...
		if ($this->debug) dfl(esc_html__('All done', 'unicaevents'));
	...
	function get_authors_from_import( $import_data ) {
		...
		if ($this->debug) dfl( esc_html__('Get authors from import', 'unicaevents') );
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					if ($this->debug) dfl( sprintf(esc_html__( 'Failed to import author "%s". Their posts will be attributed to the current user.', 'unicaevents' ), $post['post_author']) );
					continue;
				}
	...
	function get_author_mapping() {
		...
		if ($this->debug) dfl( esc_html__('Author mapping', 'unicaevents') );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ($this->debug) dfl( sprintf(esc_html__('Author "%s" imported.', 'unicaevents'), $this->authors[$old_login]['author_display_name']) );
				} else {
					if ($this->debug) dfl( sprintf(esc_html__('Failed to create new user for %s. Their posts will be attributed to the current user. Message: %s', 'unicaevents'), $this->authors[$old_login]['author_display_name'], $user_id->get_error_message()) );
				}
			}
	...
	function process_categories() {
		...
		if ($this->debug) dfl( esc_html__('Process categories', 'unicaevents') );
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($cat['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf( esc_html__( 'Category "%s" imported.', 'unicaevents' ).'<br>', esc_html($cat['category_nicename']) ) );
				}
			} else {
				dfl( sprintf( esc_html__( 'Failed to import category "%s"', 'unicaevents' ), esc_html($cat['category_nicename']), $id->get_error_message() ) );
				continue;
			}
		}
	...
	function process_tags() {
		...
		if ($this->debug) dfl( esc_html__('Process tags', 'unicaevents') );
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($tag['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf( esc_html__( 'Tag "%s" imported.', 'unicaevents' ), $tag['tag_name']) );
				}
			} else {
				dfl( sprintf( esc_html__( 'Failed to import post tag "%s". Message: %s', 'unicaevents' ), $tag['tag_name'], $id->get_error_message()) );
				continue;
			}
		}
	...
	function process_terms() {
		...
		if ($this->debug) dfl( esc_html__('Process terms', 'unicaevents') );
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($term['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf(esc_html__( 'Taxonomy "%s": term "%s" imported.', 'unicaevents' ), $term['term_taxonomy'], $term['term_name']) );
				}
			} else {
				dfl( sprintf(esc_html__( 'Failed to import term %s: "%s". Message: %s', 'unicaevents' ), $term['term_taxonomy'], $term['term_name'], $id->get_error_message()) );
				continue;
			}
		}
	...
	function process_posts() {
		...
		if ($this->debug) dfl( sprintf(esc_html__('Process posts %s', 'unicaevents'), $this->start_from_id > 0 
																						? sprintf(esc_html__('(continue from ID = %s)', 'unicaevents'), $this->start_from_id) 
																						: ''
											)
								);
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				...
				dfl( sprintf( esc_html__( 'Failed to import post "%s": Invalid post type "%s"', 'unicaevents' ), $post['post_title'], $post['post_type']) );
				...
				do {	
					...
					if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
						dfl( sprintf(esc_html__('%s "%s" already exists.', 'unicaevents'), $post_type_object->labels->singular_name, $post['post_title']) );
						...
						if ( is_wp_error( $post_id ) ) {
							dfl( sprintf(esc_html__( 'Failed to import %s: "%s". Message: %s', 'unicaevents' ), $post_type_object->labels->singular_name, $post['post_title'], $post_id->get_error_message()) );
							break;
						}
						...
						if ( $this->debug ) dfl( sprintf( esc_html__( '%s "%s" (ID=%s) imported.', 'unicaevents' ), $post_type_object->labels->singular_name, $post['post_title'], $post_id) );
					}
						...
						foreach ( $post['terms'] as $term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									//if ( $this->debug ) { dfl( sprintf( esc_html__( 'Post term %s: "%s" imported.', 'unicaevents' ).'<br>', esc_html($taxonomy), esc_html($term['name']) ) );
								} else {
									dfl( sprintf( esc_html__( 'Failed to import post term %s: "%s". Message: %s', 'unicaevents' ), $taxonomy, $term['name'], $t->get_error_message()) );
						...
						foreach ( $newcomments as $key => $comment ) {
							...
							if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
								...
								foreach( $comment['commentmeta'] as $meta ) {
									$value = unicaevents_unserialize( $meta['value'] );
							...
							if ( $key ) {
								...
								if ( ! $value ) {
									...
									if (is_serialized($value)) {
										$value = unicaevents_unserialize( $value );
										if ( $value===false ) {
											if ($this->debug) dfl( sprintf(esc_html__('Post (ID=%s) - error unserialize postmeta: %s', 'unicaevents'), $post['post_id'], $key) );
											$value = $meta['value'];
											$replace = false;
										}
									}
			...
			$admin_tm = max(0, min(1800, (int) unicaevents_get_theme_option('admin_dummy_timeout')));
			...
			unicaevents_fpc($this->import_log, trim(max($original_post_ID, $this->start_from_id)) . '|' . trim($result));
			if ($this->debug) dfl( sprintf( esc_html__('Post (ID=%s) imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'unicaevents'), $post['post_id'], $result.'%', $this->max_time, time() - $this->start_time) );
	...
	function process_menu_item( $item ) {
		...
		if ($this->debug) dfl( esc_html__('Process menu item', 'unicaevents') );
		...
		if ( ! $menu_slug ) {
			if ($this->debug) dfl( sprintf(esc_html__( 'Menu item "%s" (id=%s) skipped due to missing menu slug', 'unicaevents' ), $item['post_title'], $item['post_id']) );
			return;
		}

		$menu_id = term_exists( $menu_slug, 'nav_menu' );
		if ( ! $menu_id ) {
			if ($this->debug) dfl( sprintf( esc_html__( 'Menu item "%s" (id=%s) skipped due to invalid menu slug: %s', 'unicaevents' ), $item['post_title'], $item['post_id'], $menu_slug) );
			return;
		} else {
		...
		$_menu_item_classes = unicaevents_unserialize( $_menu_item_classes );
		...
		if ($_menu_item_type == 'custom' && strpos($_menu_item_url, $this->demo_url) !== false) {
			$_menu_item_url = str_replace($this->demo_url, get_site_url() . (unicaevents_substr($this->demo_url, -1)=='/' ? '/' : ''), $_menu_item_url);
		}
		...
		if ( $id && ! is_wp_error( $id ) ) {
			if ($this->overwrite) {
				...
				if ( ! empty( $item['postmeta'] ) ) {
					foreach ( $item['postmeta'] as $meta ) {
						...
						if (unicaevents_substr($key, 0, 11)=='_menu_item_') continue;
						...
						if ( $key ) {
							...
							if ( ! $value ) {
								...
								if (is_serialized($value)) {
									...
									if ( $value === false ) {
										if ($this->debug) {
											dfl(sprintf( esc_html__('Menu item (ID=%s) - error unserialize postmeta: %s=', 'unicaevents'), $item['post_id'], $key) );
										}
										$value = $meta['value'];
										$replace = false;
									}
			...
			if ( $this->debug ) dfl( sprintf( esc_html__( 'Menu item "%s" (ID=%s) imported.', 'unicaevents'), $item['post_title'], $item['post_id']) );
		}
	}
	...
	function process_attachment( $post, $url ) {
		...
		if ($this->debug) dfl( esc_html__('Process attachment', 'unicaevents') );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			$msg = esc_html__('Invalid file type', 'unicaevents');
			dfl( $msg );
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ($this->debug) dfl( sprintf(esc_html__('Fetch remote file "%s"', 'unicaevents'), $url) );
		...
		if ( !file_exists($upload['file']) && $this->fetch_attachments ) {
			...
			if ( ! $headers ) {
				@unlink( $upload['file'] );
				$msg = esc_html__('Remote server did not respond', 'unicaevents');
				dfl($msg);
			...
			if ( $headers['response'] != '200' ) {
				@unlink( $upload['file'] );
				$msg = sprintf( esc_html__('Remote server returned error response %1$d %2$s', 'unicaevents'), $headers['response'], get_status_header_desc($headers['response']) );
				dfl($msg);
				return new WP_Error( 'import_file_error', $msg );
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				@unlink( $upload['file'] );
				$msg = esc_html__('Remote file is incorrect size', 'unicaevents');
				dfl($msg);
				return new WP_Error( 'import_file_error', $msg );
			}
			...
			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );
				$msg = esc_html__('Zero size file downloaded', 'unicaevents');
				dfl($msg);
				return new WP_Error( 'import_file_error', $msg );
			}
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				@unlink( $upload['file'] );
				$msg = sprintf(esc_html__('Remote file is too large, limit is %s', 'unicaevents'), size_format($max_size) );
				dfl($msg);
				return new WP_Error( 'import_file_error', $msg );
			}
			...
			if ($this->debug) dfl(esc_html__('Attachment fetched', 'unicaevents'));
		} else
			if ($this->debug) dfl(esc_html__('Attachment already uploaded', 'unicaevents'));
	...
	function replace_uploads($str) {
		return unicaevents_replace_uploads_url($str, $this->uploads_folder);
	}
	...
	function backfill_parents() {
		...
		if ($this->debug) dfl(esc_html__('Backfill parents', 'unicaevents'));
	...
	function backfill_attachment_urls() {
		...
		if ($this->debug) dfl(esc_html__('Backfill attachments', 'unicaevents'));
	...
	function remap_featured_images() {
		...
		if ($this->debug) dfl(esc_html__('Remap featured images', 'unicaevents'));
	 ...
	 */
	function parse( $file ) {
		if ($this->debug) dfl( sprintf(esc_html__('Parse file %s', 'unicaevents'), $file) );
