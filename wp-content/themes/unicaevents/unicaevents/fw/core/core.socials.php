<?php
/**
 * UnicaEvents Framework: social networks
 *
 * @package	unicaevents
...
// Theme init
if (!function_exists('unicaevents_socials_theme_setup')) {
	add_action( 'unicaevents_action_before_init_theme', 'unicaevents_socials_theme_setup' );
	function unicaevents_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'unicaevents_facebook_og_tags', 5 );
		}
...
$UNICAEVENTS_GLOBALS['share_links'] = array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=unicaevents',
...
if (!function_exists('unicaevents_add_share_link')) {
	function unicaevents_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		global $UNICAEVENTS_GLOBALS;
		$UNICAEVENTS_GLOBALS['share_links'] = array_merge( $UNICAEVENTS_GLOBALS['share_links'], $soc );
	}
}

// Return (and show) share social links
if (!function_exists('unicaevents_show_share_links')) {
	function unicaevents_show_share_links($args) {
		if ( unicaevents_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => unicaevents_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => unicaevents_get_custom_option('show_share'),				// share block direction
			'caption' => unicaevents_get_custom_option('share_caption'),				// share block caption
			'share' => unicaevents_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = empty($soc['url']) && !empty($UNICAEVENTS_GLOBALS['share_links'][$sn]) ? $UNICAEVENTS_GLOBALS['share_links'][$sn] : $soc['url'];
				...
				$output .= '<div class="sc_socials_item">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(unicaevents_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('unicaevents_prepare_socials')) {
	function unicaevents_prepare_socials($list, $style='') {
		if (empty($style)) $style = unicaevents_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || unicaevents_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : unicaevents_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('unicaevents_get_twitter_data')) {
	function unicaevents_get_twitter_data($cfg) {
		return function_exists('trx_utils_get_twitter_data') 
				? trx_utils_get_twitter_data(array(
						'mode'            => 'user_timeline',
...
if (!function_exists('unicaevents_prepare_twitter_text')) {
	function unicaevents_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('unicaevents_get_twitter_followers')) {
	function unicaevents_get_twitter_followers($cfg) {
		$data = unicaevents_get_twitter_data($cfg); 
...
if (!function_exists('unicaevents_get_facebook_likes')) {
	function unicaevents_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(unicaevents_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (unicaevents_strpos($headers[0], '200')) {
			$json = unicaevents_fgc($url);
...
if (!function_exists('unicaevents_facebook_og_tags')) {
	function unicaevents_facebook_og_tags() {
		global $post;
		if ( !is_singular() || unicaevents_get_global('blog_streampage')) return;
...
if (!function_exists('unicaevents_get_feedburner_counter')) {
	function unicaevents_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(unicaevents_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (unicaevents_strpos($headers[0], '200')) {
			$xml = unicaevents_fgc($url);
