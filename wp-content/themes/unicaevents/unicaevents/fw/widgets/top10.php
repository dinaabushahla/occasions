 ...
 */
add_action( 'widgets_init', 'unicaevents_widget_top10_load' );

/**
 * Register our widget.
 */
function unicaevents_widget_top10_load() {
	register_widget('unicaevents_widget_top10');
}

/**
 * Top10 Widget class.
 */
class unicaevents_widget_top10 extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array('classname' => 'widget_top10', 'description' => esc_html__('Top 10 posts by average reviews marks (by author and users)', 'unicaevents'));

		/* Widget control settings. */
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'unicaevents_widget_top10');

		/* Create the widget. */
		parent::__construct( 'unicaevents_widget_top10', esc_html__('UnicaEvents - Top 10 Posts', 'unicaevents'), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		...
		if (count($tabs) > 0) {
			...
			if (count($tabs) > 1) unicaevents_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
			...
			echo count($tabs) == 1 ? $tabs[0]['content'] : unicaevents_do_shortcode('[trx_tabs style="2"][trx_tab title="'.esc_attr($tabs[0]['title']).'"]'.($tabs[0]['content']).'[/trx_tab][trx_tab title="'.esc_attr($tabs[1]['title']).'"]'.($tabs[1]['content']).'[/trx_tab][/trx_tabs]');
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_author')); ?>"><?php esc_html_e('Author rating tab title:', 'unicaevents'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_users')); ?>"><?php esc_html_e('Users rating tab title:', 'unicaevents'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2"><?php esc_html_e('As stars', 'unicaevents'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('As icon', 'unicaevents'); ?></label>
